...
struct urTwoQueueLaunchBlockingFree : uur::urMultiQueueMultiDeviceTest {
    std::string KernelNameWait;
    std::string KernelNameStore;
    std::vector<ur_program_handle_t> programs;
    std::vector<ur_kernel_handle_t> kernels;
    std::vector<void *> SharedMem;

    static constexpr char ProgramNameWait[] = "atomic_wait";
    static constexpr char ProgramNameStore[] = "atomic_store";
    ...
    void SetUp() override {
        ...
        KernelNameWait = uur::KernelsEnvironment::instance->GetEntryPointNames(
            ProgramNameWait)[0];
        KernelNameStore = uur::KernelsEnvironment::instance->GetEntryPointNames(
            ProgramNameStore)[0];
        ...
        for (size_t i = 0; i < devices.size(); i++) {
            ...
            auto ProgramName = i % 2 == 0 ? ProgramNameWait : ProgramNameStore;
            auto KernelName = i % 2 == 0 ? KernelNameWait : KernelNameStore;
            ...
            if (i % 2 == 0) {
                ...
                ASSERT_SUCCESS(
                    urKernelSetArgPointer(kernels[i], 0, nullptr, SharedMem[i - 1]));
            }
        }
    }
...
TEST_F(urTwoQueueLaunchBlockingFree, FreeDoesNotDeadlock) {
    ...
    auto signalKernel = [&](size_t i) {
        ...
        auto signalIndex = (i + 1) % queues.size();

        ASSERT_SUCCESS(urEnqueueKernelLaunch(queues[signalIndex], kernels[signalIndex], 1,
                                         &global_offset, &global_size, nullptr,
                                         0, nullptr, nullptr));
        ASSERT_SUCCESS(urQueueFinish(queues[signalIndex]));
    };
