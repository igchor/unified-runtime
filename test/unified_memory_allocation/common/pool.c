...
static enum uma_result_t nullInitialize(struct uma_memory_provider_desc_t *providers,
...
uma_memory_pool_handle_t nullPoolCreate(void) {
    ...
    struct uma_memory_provider_desc_t providerDesc = {
        .hProvider = nullProviderCreate(),
        .providerType = UMA_MEMORY_PROVIDER_TYPE_DATA_AND_METADATA};
...
static enum uma_result_t traceInitialize(struct uma_memory_provider_desc_t *providers,
                                         size_t numProviders, void *params, void **pool) {
    struct tracePool *tracePool = (struct tracePool *)malloc(sizeof(struct tracePool));
...
static void *traceAlignedMalloc(void *pool, size_t size, size_t alignment) {
    ...
    return umaPoolAlignedMalloc(tracePool->params.hUpstreamPool, size, alignment);
}
...
uma_memory_pool_handle_t tracePoolCreate(uma_memory_pool_handle_t hUpstreamPool, void (*trace)(const char *)) {
    ...
    uma_memory_pool_handle_t hPool;
    enum uma_result_t ret = umaPoolCreate(&ops, &providerDesc, 1, &params, &hPool);
