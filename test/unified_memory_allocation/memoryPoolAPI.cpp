...
TEST_F(test, memoryPoolWithCustomProviders) {
    ...
    struct pool : public uma_test::pool_base {
        uma_result_t initialize(struct uma_memory_provider_desc_t *providers,
            ...
            EXPECT_EQ(providers[1].providerType,
                      UMA_MEMORY_PROVIDER_TYPE_METADATA);
...
makePool(std::function<uma::provider_unique_handle_t()> makeProvider) {
    ...
    uma_memory_provider_desc_t desc{provider.get(),
                                    UMA_MEMORY_PROVIDER_TYPE_DATA};
    auto pool = uma::poolMakeUnique<Pool>(&desc, 1).second;
    auto dtor = [provider =
                     provider.release()](uma_memory_pool_handle_t hPool) {
...
TEST_F(test, memoryPoolInvalidProvidersType) {
    auto nullProvider = uma_test::wrapProviderUnique(nullProviderCreate());
    struct uma_memory_provider_desc_t providers[] = {
        {nullProvider.get(), UMA_MEMORY_PROVIDER_TYPE_METADATA}};
