  ...
  if (LocalWorkSize) {
    ...
    if (SuggestGroupSize) {
      ZE2UR_CALL(zeKernelSuggestGroupSize,
                 (Kernel->getZeKernel(Device), GlobalWorkSize3D[0],
                  GlobalWorkSize3D[1], GlobalWorkSize3D[2], &WG[0], &WG[1],
                  &WG[2]));
    } else {
      for (int I : {0, 1, 2}) {
...
[[nodiscard]] static ur_result_t urEnqueueKernelLaunchV2(
    v2::ur_queue_dispatcher_t *Queue, ///< [in] handle of the queue object
  ...
  ze_kernel_handle_t ZeKernel = Kernel->getZeKernel(Device);
  if (!ZeKernel) {
  ...
  if (GlobalWorkOffset != NULL) {
    if (!Device->Platform->ZeDriverGlobalOffsetExtensionFound) {
  ...
  UR_CALL(calculateKernelWorkDimensions(Kernel, Device, ZeThreadGroupDimensions,
  ...
  auto L0Enqueue = [&](ze_command_list_handle_t CommandList,
    ...
    ZE2UR_CALL_THROWS(zeCommandListAppendLaunchKernel,
                      (CommandList, ZeKernel, &ZeThreadGroupDimensions,
                       SignalEvent, WaitListLength, WaitList));
  };

  Queue->enqueue(OutEvent, NumEventsInWaitList, EventWaitList,
...
UR_APIEXPORT ur_result_t UR_APICALL urEnqueueKernelLaunch(
  ...
  if (UrQueue->is_dispatcher()) {
    return urEnqueueKernelLaunchV2(
        Dispatcher(UrQueue), Kernel, WorkDim, GlobalWorkOffset, GlobalWorkSize,
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelCreate(
  ...
  auto UrKernel = std::make_unique<ur_kernel_handle_t_>(true, Program);

  for (auto [ZeDevice, ZeModule] : Program->ZeModuleMap) {
    ZeStruct<ze_kernel_desc_t> ZeKernelDesc;
    ZeKernelDesc.pKernelName = KernelName;

    ze_kernel_handle_t ZeKernel;
    ZE2UR_CALL(zeKernelCreate, (ZeModule, &ZeKernelDesc, &ZeKernel));
    UrKernel->ZeKernels.emplace_back(ZeDevice, ZeKernel);
  }

  UR_CALL(UrKernel->initialize());

  *RetKernel = reinterpret_cast<ur_kernel_handle_t>(UrKernel.release());
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelSetArgValue(
  ...
  for (auto [_, ZeKernel] : Kernel->ZeKernels) {
    ZE2UR_CALL(zeKernelSetArgumentValue,
  ...
  case UR_KERNEL_INFO_ATTRIBUTES:
    try {
      ...
      ZE2UR_CALL(zeKernelGetSourceAttributes,
                 (Kernel->ZeKernels[0].second, &Size, nullptr));
      char *attributes = new char[Size];
      ZE2UR_CALL(zeKernelGetSourceAttributes,
                 (Kernel->ZeKernels[0].second, &Size, &attributes));
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelGetGroupInfo(
  ...
  case UR_KERNEL_GROUP_INFO_WORK_GROUP_SIZE: {
    ...
    auto ZeKernel = Kernel->getZeKernel(Device);
    if (!ZeKernel) {
      return UR_RESULT_ERROR_INVALID_DEVICE;
    }

    auto ZeResult =
        ZE_CALL_NOCHECK(zeKernelGetProperties, (ZeKernel, &kernelProperties));
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelSetExecInfo(
  ...
  assert(Kernel->ZeKernels.size());
  auto ZeKernel = Kernel->ZeKernels[0].second;
  std::scoped_lock<ur_shared_mutex> Guard(Kernel->Mutex);
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelSetArgSampler(
  ...
  std::scoped_lock<ur_shared_mutex> Guard(Kernel->Mutex);
  assert(Kernel->ZeKernels.size());
  ZE2UR_CALL(zeKernelSetArgumentValue, (Kernel->ZeKernels[0].second, ArgIndex,
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelGetNativeHandle(
  ...
  assert(Kernel->ZeKernels.size());
  *NativeKernel =
      reinterpret_cast<ur_native_handle_t>(Kernel->ZeKernels[0].second);
  return UR_RESULT_SUCCESS;
}
...
ur_result_t ur_kernel_handle_t_::initialize() {
  ...
  ZeKernelProperties.Compute = [this](ze_kernel_properties_t &Properties) {
    assert(ZeKernels.size());
    ZE_CALL_NOCHECK(zeKernelGetProperties, (ZeKernels[0].second, &Properties));
  };
  ...
  ZeKernelName.Compute = [this](std::string &Name) {
    assert(ZeKernels.size());
    size_t Size = 0;
    ZE_CALL_NOCHECK(zeKernelGetName, (ZeKernels[0].second, &Size, nullptr));
    char *KernelName = new char[Size];
    ZE_CALL_NOCHECK(zeKernelGetName, (ZeKernels[0].second, &Size, KernelName));
    Name = KernelName;
    delete[] KernelName;
  };
