...
ur_result_t urUSMPitchedAllocExp(ur_context_handle_t hContext,
  ...
  UR_CALL(callDriverFunction<zeMemGetPitchFor2dImage_pfn>(
      hContext->getPlatform()->ZeDriver, hContext->ZeContext,
      ZeDeviceTranslated, Width, height, elementSizeBytes, &RowPitch));
...
ur_result_t urBindlessImagesUnsampledImageCreateExp(
  ...
  UR_CALL(bindlessImageCreate(hContext, hDevice, hImageMem, pImageFormat,
                                   pImageDesc, nullptr, bindlessHandle));

  *phImage = bindlessHandle.DeviceOffset;
  hDevice->ZeOffsetToImageHandleMap[*phImage] = bindlessHandle.ZeImage;
...
ur_result_t urBindlessImagesSampledImageCreateExp(
  ...
  UR_CALL(bindlessImageCreate(hContext, hDevice, hImageMem, pImageFormat,
                                   pImageDesc, hSampler, bindlessHandle));
