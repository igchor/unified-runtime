...
static std::pair<ze_image_format_type_t, size_t>
...
static uint32_t getPixelSizeBytes(const ur_image_format_t *Format) {
...
// TODO: unify with ur2zeImageDesc
static ur_result_t
...
image_copy_desc_t getImageCopyDesc(ur_exp_image_copy_flags_t imageCopyFlags,
  ...
  ZeStruct<ze_image_desc_t> ZeImageDesc;
  UR_CALL_THROWS(
      ur2zeImageDescBindless(pSrcImageFormat, pSrcImageDesc, ZeImageDesc));

  image_copy_desc_t ImageCopyDesc;
  ...
  if (imageCopyFlags == UR_EXP_IMAGE_COPY_FLAG_HOST_TO_DEVICE) {
    ...
    if (pDstImageDesc->rowPitch == 0) {
      ...
      UR_CALL_THROWS(getImageRegionHelper(ZeImageDesc, &pCopyRegion->dstOffset,
      ...
      ImageCopyDesc =
          h2d_non_usm{reinterpret_cast<const void *>(SrcPtr),
                      reinterpret_cast<ur_exp_image_native_handle_t>(pDst),
                      SrcRowPitch, SrcSlicePitch, DstRegion};
    } else {
      ...
      ze_copy_region_t ZeSrcRegion = {(uint32_t)pCopyRegion->srcOffset.x,
                                      ...
                                      (uint32_t)pCopyRegion->copyExtent.height,
                                      (uint32_t)pCopyRegion->copyExtent.depth};
      ImageCopyDesc = h2d_pitched_usm{reinterpret_cast<const void *>(pSrc),
                                      pDst,
                                      SrcRowPitch,
                                      SrcSlicePitch,
                                      DstRowPitch,
    ...
    if (pSrcImageDesc->rowPitch == 0) {
      ...
      UR_CALL_THROWS(getImageRegionHelper(ZeImageDesc, &pCopyRegion->srcOffset,
      ...
      ImageCopyDesc =
          d2h_non_usm{reinterpret_cast<ur_exp_image_native_handle_t>(pSrc),
                      reinterpret_cast<void *>(DstPtr), SrcRegion, DstRowPitch,
                      DstSlicePitch};
    } else {
      ...
      ImageCopyDesc = d2h_pitched_usm{reinterpret_cast<const void *>(pSrc),
                                      pDst,
                                      SrcRowPitch,
                                      ZeSrcRegion,
                                      SrcSlicePitch,
    ...
    UR_CALL_THROWS(getImageRegionHelper(ZeImageDesc, &pCopyRegion->srcOffset,
                                        &pCopyRegion->copyExtent, SrcRegion));
    ImageCopyDesc = d2d{reinterpret_cast<ur_exp_image_native_handle_t>(pSrc),
                        reinterpret_cast<ur_exp_image_native_handle_t>(pDst),
                        SrcRegion, DstRegion};
  } else {
  ...
  return ImageCopyDesc;
}

static ur_result_t bindlessImagesCreateImpl(
  ...
  ZE2UR_CALL(zeMemGetAllocProperties,
             (hContext->getZeHandle(),
              reinterpret_cast<const void *>(hImageMem), &MemAllocProperties,
              nullptr));
  if (MemAllocProperties.type == ZE_MEMORY_TYPE_UNKNOWN) {
    auto *UrImage = reinterpret_cast<ur_exp_bindless_image_t *>(hImageMem);

    ZE2UR_CALL(zeImageViewCreateExt,
               (hContext->getZeHandle(), hDevice->ZeDevice, &ZeImageDesc,
                UrImage->getZeImage(), &ZeImage));
...
ur_result_t urBindlessImagesImageAllocateExp(
  ...
  *phImageMem = reinterpret_cast<ur_exp_image_mem_native_handle_t>(
      new ur_exp_bindless_image_t(ZeImage,
                                  {ZeImageDesc.width, ZeImageDesc.height,
                                   ZeImageDesc.depth, *pImageFormat}));
  return UR_RESULT_SUCCESS;
}
...
ur_result_t urBindlessImagesImageGetInfoExp(
  ...
  auto *UrImage = reinterpret_cast<ur_exp_bindless_image_t *>(hImageMem);
  auto Info = UrImage->getInfo();
  switch (propName) {
  case UR_IMAGE_INFO_WIDTH:
    if (pPropValue) {
      *(uint64_t *)pPropValue = Info.width;
    }
    if (pPropSizeRet) {
  ...
  case UR_IMAGE_INFO_HEIGHT:
    if (pPropValue) {
      *(uint32_t *)pPropValue = Info.height;
    }
    if (pPropSizeRet) {
  ...
  case UR_IMAGE_INFO_DEPTH:
    if (pPropValue) {
      *(uint32_t *)pPropValue = Info.depth;
    }
    if (pPropSizeRet) {
      *pPropSizeRet = sizeof(uint32_t);
    }
  ...
  case UR_IMAGE_INFO_FORMAT:
    if (pPropValue) {
      *(ur_image_format_t *)pPropValue = Info.format;
    }
    if (pPropSizeRet) {
