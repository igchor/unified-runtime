...
inline ur_result_t deferredEventRetain(ur_event_handle_t hEvent) {
  assert(hEvent);
  assert(reinterpret_cast<_ur_object *>(hEvent)->RefCount.load() == 0);
  return UR_RESULT_SUCCESS;
}

inline ur_result_t defferedEventRelease(ur_event_handle_t hEvent) {
  assert(hEvent);
  assert(reinterpret_cast<_ur_object *>(hEvent)->RefCount.load() == 0);
  return UR_RESULT_SUCCESS;
}

// deferredEvents has refCount equal to 0, the only operation that can be called
// on them is releaseDeferred()
