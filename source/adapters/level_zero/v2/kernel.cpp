...
ur_kernel_handle_t_::getZeHandle(ur_device_handle_t hDevice) {
  ...
  if (!deviceKernels[hDevice->Id].has_value()) {
    throw UR_RESULT_ERROR_INVALID_DEVICE;
  }

  assert(deviceKernels[hDevice->Id].value().hKernel.get());

  return deviceKernels[hDevice->Id].value().hKernel.get();
}
...
ur_kernel_handle_t_::getProperties(ur_device_handle_t hDevice) const {
  ...
  return *deviceKernels[hDevice->Id].value().zeKernelProperties.operator->();
}
