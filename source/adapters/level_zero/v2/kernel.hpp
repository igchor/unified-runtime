...
public:
  ...
  void addPendingMemoryAllocation(pending_memory_allocation_t allocation);
  ...
  prepareForSubmission(ur_context_handle_t hContext, ur_device_handle_t hDevice,
                       const size_t *pGlobalWorkOffset, uint32_t groupSizeX,
...
private:
  ...
  const size_t deviceIndex(ur_device_handle_t hDevice) const;
