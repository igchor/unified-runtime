...
static umf_usm_memory_type_t urToUmfMemoryType(ur_usm_type_t type) {
  switch (type) {
...
UR_APIEXPORT ur_result_t UR_APICALL urUSMSharedAlloc(
  ...
  dev_desc.flags = 0;
  dev_desc.ordinal = 0;
  ZE2UR_CALL(zeMemAllocShared, (hContext->getZeHandle(), &dev_desc, &host_desc,
  ...
  // // TODO: support sharedReadOnly
  // return hPool->allocate(hContext, hDevice, pUSMDesc, UR_USM_TYPE_SHARED,
  // size,
  //                        ppRetMem);
}
