...
ur_mem_buffer_t::ur_mem_buffer_t(ur_context_handle_t hContext, size_t size,
    ...
    : ur_mem_handle_t_(hContext), accessMode(accessMode), size(size) {}

size_t ur_mem_buffer_t::getSize() const { return size; }
...
getSyncCommandListForCopy(ur_context_handle_t hContext,
  ...
  return hContext->commandListCache.getImmediateCommandList(
...
ur_mem_sub_buffer_t::ur_mem_sub_buffer_t(ur_mem_buffer_t *hParent,
                                         size_t offset, size_t size,
                                         device_access_mode_t accessMode)
    : ur_mem_buffer_t(hParent->getContext(), size, accessMode),
...
ur_mem_image_t::ur_mem_image_t(ur_context_handle_t hContext,
    ...
    : ur_mem_handle_t_(hContext), zeImage(zeImage, ownZeImage) {
...
ur_result_t urMemBufferCreate(ur_context_handle_t hContext,
  ...
  if (useHostBuffer(hContext)) {
    ...
    *phBuffer = new ur_integrated_buffer_handle_t(hContext, hostPtr, size,
                                                  hostPtrAction, accessMode);
  } else {
    *phBuffer =
        new ur_discrete_buffer_handle_t(hContext, hostPtr, size, accessMode);
  }
...
ur_result_t urMemBufferCreateWithNativeHandle(
  ...
  if (useHostBuffer(hContext) && memoryAttrs.type == ZE_MEMORY_TYPE_HOST) {
    *phMem = new ur_integrated_buffer_handle_t(hContext, ptr, size, accessMode,
                                               ownNativeHandle);
    ...
    if (memoryAttrs.type == ZE_MEMORY_TYPE_HOST) {
      ...
      *phMem = new ur_discrete_buffer_handle_t(
...
ur_result_t urMemGetInfo(ur_mem_handle_t hMem, ur_mem_info_t propName,
  ...
  auto hMemory = dynamic_cast<ur_mem_buffer_t *>(hMem);
  UR_ASSERT(hMemory, UR_RESULT_ERROR_INVALID_MEM_OBJECT);
...
ur_result_t urMemImageCreate(ur_context_handle_t hContext, ur_mem_flags_t flags,
  ...
  *phMem = new ur_mem_image_t(hContext, flags, pImageFormat, pImageDesc, pHost);
  return UR_RESULT_SUCCESS;
} catch (...) {
  return exceptionToResult(std::current_exception());
}
...
ur_result_t urMemImageCreateWithNativeHandle(
  ...
  ur_result_t res = ur2zeImageDesc(pImageFormat, pImageDesc, zeImageDesc);
  if (res != UR_RESULT_SUCCESS) {
    *phMem = nullptr;
    return res;
  }

  *phMem = new ur_mem_image_t(hContext, pImageFormat, pImageDesc, zeImage,
                              ownNativeHandle);
  return UR_RESULT_SUCCESS;
} catch (...) {
  return exceptionToResult(std::current_exception());
