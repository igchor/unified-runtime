...
ur_integrated_mem_handle_t::ur_integrated_mem_handle_t(
  ...
  if (hostPtrImported) {
    this->ptr = std::unique_ptr<void, std::function<void(void *)>>(
        hostPtr, [hContext](void *ptr) {
...
ur_integrated_mem_handle_t::ur_integrated_mem_handle_t(
    ...
    : ur_mem_handle_t_(hContext, size) {
  this->ptr = std::unique_ptr<void, std::function<void(void *)>>(
      hostPtr, [hContext, ownHostPtr](void *ptr) {
        if (!ownHostPtr) {
          return;
        }
        auto ret = hContext->getDefaultUSMPool()->free(ptr);
...
void *ur_discrete_mem_handle_t::allocateOnDevice(ur_device_handle_t hDevice,
  ...
  deviceAllocations[id] = std::unique_ptr<void, std::function<void(void *)>>(
      ptr, [hContext = this->hContext](void *ptr) {
        auto ret = hContext->getDefaultUSMPool()->free(ptr);
        if (ret != UR_RESULT_SUCCESS) {
...
ur_discrete_mem_handle_t::ur_discrete_mem_handle_t(ur_context_handle_t hContext,
    ...
    : ur_mem_handle_t_(hContext, size),
      deviceAllocations(hContext->getPlatform()->getNumDevices()),
      activeAllocationDevice(hDevice), hostAllocations() {
  assert(hDevice);
  assert(devicePtr);
...
ur_result_t urMemBufferCreateWithNativeHandle(
  ...
  auto [base, size] = getMemoryBaseAddressAndSize(hContext->getZeHandle(), ptr);
  UR_ASSERT(ptr == base, UR_RESULT_ERROR_INVALID_VALUE);

  ze_device_handle_t zeDevice;
  auto memoryAttrs = getMemoryAttrs(hContext->getZeHandle(), ptr, &zeDevice);
  ...
  // TODO: legacy adapter copies back the content to the native ptr on buffer
  // dtor is this actually necessary?
  if (useHostBuffer(hContext) && memoryAttrs.type == ZE_MEMORY_TYPE_HOST) {
    *phMem =
    ...
    if (memoryAttrs.type == ZE_MEMORY_TYPE_HOST) {
      // cannot use host memory to create a buffer on discrete devices
      return UR_RESULT_ERROR_INVALID_VALUE;
    }
