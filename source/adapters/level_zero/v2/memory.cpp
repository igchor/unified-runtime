...
void *ur_discrete_mem_handle_t::mapHostPtr(
  ...
  hostAllocations.emplace_back();
  hostAllocations.back().offset = offset;
  hostAllocations.back().size = size;
  hostAllocations.back().access = access;
  UR_CALL_THROWS(hContext->getDefaultUSMPool()->allocate(
      hContext, nullptr, nullptr, UR_USM_TYPE_HOST, size,
      &hostAllocations.back().ptr));
