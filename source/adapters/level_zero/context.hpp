...
struct ur_context_handle_t_ : _ur_object {
  ur_context_handle_t_(ze_context_handle_t ZeContext, uint32_t NumDevices,
    ...
    for (uint32_t i = 0; i < NumDevices; i++) {
      V2EventPool[Devs[i]->ZeDevice] = std::make_unique<v2::ur_event_pool_t>(
          ZeContext, Devs[i]->ZeDevice, 256);
    }
  ...
  // TODO: Create abstraction for this (and change unordered_map to vector -
  // index each device in the context by integer)
  std::unordered_map<ze_device_handle_t, std::unique_ptr<v2::ur_event_pool_t>>
      V2EventPool;
