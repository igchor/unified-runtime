...
/// @brief creates UMA memory tracker based on given Tracker type.
/// Tracker should implement all functions defined by
/// uma_memory_tracker_ops_t, except for intialize and finalize (those are
/// replaced by ctor and dtor). All arguments passed to this function are
/// forwarded to Tracker ctor.
template <typename Tracker, typename... Args>
auto trackerMakeUnique(Args &&...args) {
    uma_memory_tracker_ops_t ops;

    UMA_OPS_GENERATE_FUNCTION(Tracker, add, ops);
    UMA_OPS_GENERATE_FUNCTION(Tracker, remove, ops);
    UMA_OPS_GENERATE_FUNCTION(Tracker, find, ops);

    return createUmaObject<Tracker>(ops, &umaMemoryTrackerCreate, &umaMemoryTrackerDestroy, args...);
}
} // namespace uma
