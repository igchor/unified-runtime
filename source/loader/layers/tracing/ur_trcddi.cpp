...
__urdlllocal ur_result_t UR_APICALL urProgramCompileExp(
    ...
    ur_program_compile_exp_params_t params = {&hContext, &hProgram, &numDevices,
                                              &phDevices, &pOptions};
...
__urdlllocal ur_result_t UR_APICALL urProgramLinkExp(
    ...
    ur_program_link_exp_params_t params = {&hContext,   &count,     &phPrograms,
                                           &numDevices, &phDevices, &pOptions,
                                           &phProgram};
    uint64_t instance = context.notify_begin(UR_FUNCTION_PROGRAM_LINK_EXP,
                                             "urProgramLinkExp", &params);

    ur_result_t result = pfnLinkExp(hContext, count, phPrograms, numDevices,
                                    phDevices, pOptions, phProgram);
